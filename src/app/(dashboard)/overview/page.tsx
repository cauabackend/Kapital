...
import { fetchOverviewData, BudgetSummary } from "./actions";
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-2 min-w-0">
          ...
          <p className={cn("font-heading text-2xl font-semibold tracking-tight tabular-nums truncate", accent)}>
            {value}
          </p>
...
function ChartWidget({ chartData }: { chartData: Parameters<typeof SpendingChart>[0]["data"] }) {
  return (
  ...
  return (
    <div className="rounded-2xl border border-border/50 bg-card p-6">
      <div className="mb-4 flex items-center gap-2">
        <PiggyBank className="h-4 w-4 text-muted-foreground/60" strokeWidth={1.6} />
        <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60">
          Savings pots
        </p>
      </div>
        ...
        <>
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground/60">
              of {fmt(totalTarget)} target · {overallPct.toFixed(0)}%
            </p>
        ...
        <>
          ...
          <div className="mb-4 space-y-1.5">
            <div className="flex justify-between text-xs text-muted-foreground/60">
              <span>
                {paid.length} paid · {unpaid.length} remaining
              </span>
          ...
          {unpaid.length === 0 && (
            <div className="flex items-center gap-2 rounded-xl border border-[hsl(var(--income)/0.2)] bg-[hsl(var(--income)/0.05)] px-3 py-2.5">
              <Check className="h-4 w-4 text-[hsl(var(--income))]" strokeWidth={2.5} />
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          subtext={isPositiveBalance ? "All-time surplus" : "All-time deficit"}
          icon={Landmark}
          accent={isPositiveBalance ? "text-[hsl(var(--income))]" : "text-[hsl(var(--expense))]"}
          highlight
        />
        <StatCard
          label="Income"
        ...
        <StatCard
          label="Bills pending"
          value={String(data.billsPending)}
          subtext={data.billsPending === 0 ? "All clear" : "Awaiting payment"}
          icon={Receipt}
          accent={data.billsPending > 0 ? "text-amber-400" : undefined}
        />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2">
          <BudgetsWidget budgets={data.topBudgets} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
          <PotsWidget pots={data.pots} />
          <BillsWidget bills={data.bills} />
        </div>
      </div>
    </div>
  );
