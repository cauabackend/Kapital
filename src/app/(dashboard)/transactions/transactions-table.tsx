...
const SORT_OPTIONS: { label: string; field: SortField; order: SortOrder }[] = [
  ...
  { label: "Amount (highest)", field: "amount", order: "desc" },
  { label: "Amount (lowest)", field: "amount", order: "asc" },
  { label: "Description (A–Z)", field: "description", order: "asc" },
  { label: "Description (Z–A)", field: "description", order: "desc" },
];
...
export function TransactionsTable({ initialData, categories }: TransactionsTableProps) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="relative min-w-[200px] flex-1">
          <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground/40" strokeWidth={1.8} />
          <input
            type="text"
            value={searchInput}
        ...
        <div className="relative" ref={sortRef}>
          ...
          {showSort && (
            <div className="absolute right-0 top-12 z-20 w-48 overflow-hidden rounded-xl border border-border/50 bg-popover shadow-xl">
              {SORT_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    key={key}
                    onClick={() => { updateParams({ sort: key }); setShowSort(false); }}
                    className={cn(
        ...
        <div className="relative" ref={filterRef}>
          ...
          {showFilter && (
            <div className="absolute right-0 top-12 z-20 w-56 overflow-hidden rounded-xl border border-border/50 bg-popover p-3 shadow-xl">
              {/* Type filter */}
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/50">Type</p>
              ...
              {/* Category filter */}
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/50">Category</p>
              <div className="max-h-44 space-y-0.5 overflow-y-auto">
                <button
                  onClick={() => updateParams({ category: null })}
                  className={cn(
                    "flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors",
                    !categoryFilter ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50"
                  )}
                >
                  All categories
                ...
                {categories.map((cat) => (
                  <button
                    key={cat}
                    onClick={() => { updateParams({ category: cat }); setShowFilter(false); }}
                    className={cn(
                      "flex w-full items-center justify-between rounded-lg px-3 py-2 text-sm transition-colors",
                      categoryFilter === cat ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50"
                    )}
                  >
                    {cat}
                    {categoryFilter === cat && <Check className="h-3.5 w-3.5" />}
                  </button>
                ))}
              </div>
              ...
              {hasActiveFilters && (
                <button
                  onClick={() => { updateParams({ type: null, category: null }); setShowFilter(false); }}
      ...
      {/* Table */}
      <div className={cn("overflow-hidden rounded-xl border border-border/50 bg-card transition-opacity", isPending && "opacity-60")}>
        {/* Header */}
        <div className="grid grid-cols-[1fr_130px_110px_110px] items-center gap-4 border-b border-border/50 px-5 py-3">
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Description</span>
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Category</span>
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Date</span>
          <span className="text-right text-[11px] font-semibold uppercase tracking-[0.15em] text-muted-foreground/50">Amount</span>
        </div>
        ...
        {isPending ? (
          ...
          <div className="flex h-64 items-center justify-center">
            <div className="text-center">
              <p className="font-heading text-base font-medium text-muted-foreground/40">No transactions found</p>
              <p className="mt-1 text-sm text-muted-foreground/30">Try adjusting your search or filters</p>
            </div>
          </div>
        ) : (
          data.map((tx) => (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              disabled={page <= 1}
              className="flex h-9 w-9 items-center justify-center rounded-lg border border-border/50 bg-card text-muted-foreground transition-colors hover:border-border hover:text-foreground disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === "..." ? (
                  <span key={`ellipsis-${i}`} className="flex h-9 w-9 items-center justify-center text-xs text-muted-foreground/40">…</span>
                ) : (
                  <button
                    key={p}
